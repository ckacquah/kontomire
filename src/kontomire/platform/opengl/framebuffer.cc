...
static void create_texture(bool multisampled, uint32_t* out_id,
                           uint32_t count) {
...
static void attach_depth_texture(uint32_t id, int samples, GLenum format,
...
void OpenGLFramebuffer::invalidate() {
    ...
    if (_color_attachment_specification.size()) {
        ...
        Utils::create_texture(multisample, _color_attachment.data(),
                              _color_attachment_specification.size());
        ...
        for (size_t i = 0; i < _color_attachment_specification.size(); i++) {
            ...
            case FramebufferTextureFormat::RGBA8:
                Utils::attach_color_texture(
                    _color_attachment[i], _specification.samples, GL_RGBA8,
                    GL_RGBA, _specification.width, _specification.height, i);
                break;
            case FramebufferTextureFormat::RED_INTEGER:
                Utils::attach_color_texture(
                    _color_attachment[i], _specification.samples, GL_R32I,
                    GL_RED_INTEGER, _specification.width, _specification.height,
                    i);
                break;
            default:
                break;
    ...
    if (_depth_attachment_specification.texture_format !=
        ...
        if (_depth_attachment_specification.texture_format ==
            ...
            Utils::attach_depth_texture(
                _depth_attachment, _specification.samples, GL_DEPTH24_STENCIL8,
                GL_DEPTH_STENCIL_ATTACHMENT, _specification.width,
...
void OpenGLFramebuffer::resize(uint32_t width, uint32_t height) {
    if (width == 0 || height == 0 || width > _MAX_FRAMEBUFFERSIZE ||
        height > _MAX_FRAMEBUFFERSIZE) {
        return;
    }
...
void OpenGLFramebuffer::clear_attachment(uint32_t attachmentIndex, int value) {
    ...
    glClearTexImage(_color_attachment[attachmentIndex], 0,
                    Utils::texture_format_to_GL(spec.texture_format), GL_INT,
                    &value);
}
